...
1.	Evaluate the following:
   ...
   b.	ln(0) = log(0) = -Inf
   c.	ln(e) = log(exp(1)) = 1
   d.	ln(-5) = log(-5 + 0i) = 1.609438+3.141593i # result is a complex number so need to add 0i to log function; otherwise, we get NaN
   e.	-ln(5) = -log(5) = -1.609438
   f.	ln(1/5) = log(1/5) = -1.609438
...
2.	Looking back at the Triginometry section, why are we multiplying the results of the atan and atan2 functions by 180/pi? 
    By default, the output of trig functions in R are in radians. Multiplying by 180/pi converts the output to degrees.

3.	What is the difference between log and log10? (hint: use `help()`)
    log10() will compute common (base 10) logs. log() will compute natural logs (base e) by default, but we can specify another base in the function call. Since log10 is a common log, it is a built-in function in R. 
...
x <- 5
y <- 13
h <- sqrt(x^2 + y^2) # pythagorean theorem
h
```
```
...
sum() will output one value that's the sum of all values in a vector. cumsum() will output multiple values, each a cumulative sum of the values in the vector up to that position. 
...
seq(-4.8, -3.43, length=8)
  ...
  a <- seq(-4.8, -3.43, length=8)
  b <- diff(a)
  b
  0.1957143 0.1957143 0.1957143 0.1957143 0.1957143 0.1957143 0.1957143
...
```
```{r}
ppl <- c("Imogen", "Hillary", "Anna", "Meredith")
```
...
# c
tad <- dat |> filter(tadpoles >= 3 & tadpoles <= 5) # assigns new row numbers
tad
new_object <- dat[dat$tadpoles >= 3 & dat$tadpoles <= 5,] # keeps og row numbers
new_object
...
airtemp_mean <- met |> 
  group_by(date) |> 
  summarise(airtemp_mean = mean(AirTemp))
```
...
Questions
  ...
  ```
```{r}
hist(dat$frogs[dat$color == "blue"], col="lightblue", main="Blue Frogs", xlab="Number of Frogs", ylab="Frequency")
```
```
...
```{r}
# yearly
plot(met$time, met$ShortWave, main = "Shortwave radiation (2013)", xlab = "Time", ylab = "Shortwave radiation (W/m^2)")
# a day
plot(met$time[1:24], met$ShortWave[1:24], main = "Shortwave radiation (Jan 30, 2013)", xlab = "Time", ylab = "Shortwave radiation (W/m^2)")
```
...
```
```{r}
plot(met$ShortWave, met$RelHum, main = "Shortwave radiation vs. Relative Humidity", xlab = "Shortwave radiation (W/m^2)", ylab = "Relative Humidity (%)")
```
...
```
```{r}
# put airtemp into X axis 

library(ggplot2)

ggplot(met) +
 aes(x = AirTemp) +
 geom_histogram(bins = 30L, fill = "#8FABDC") +
 labs(x = "Air Temperature (Celsius)", 
 y = "Frequency ", title = "Air Temperature Histogram") +
 theme_bw()
```
```
...
The line/scatter plot has an additional variable in aes, geom_line is specific to drawing a line plot (linewidth vs bins), font modifications, etc. 
```
```{r}
library(ggplot2)
...
ggplot(met) +
 ...
 geom_line(linewidth = 0.4, colour = "#0A1425") +
 labs(x = "Date", 
 y = "Air Temperature (Celsius)", title = "Daily Air Temperature (2013)") +
 theme_bw() +
 theme(axis.text.y = element_text(size = 14L), 
 axis.text.x = element_text(size = 14L), legend.text = element_text(face = "italic", size = 14L), 
 legend.title = element_text(face = "italic", size = 14L))
```
...
# b
plot(red_frogs$frogs, red_frogs$tadpoles, col = "red", xlab = "Frog Density", ylab = "Tadpole Density", main = "Red Frogs")
abline(red_lm, col = "black", lwd = 1)
```
...
x <- 10 # added for testing
y <- 5 # added for testing
...
```
```{r}
ribbit_fun <- function(data){
  
  frogs <- data$frogs
  tadpoles <- data$tadpoles
  color <- data$color
  spots <- data$spots
  ...
  plot(frogs,tadpoles,
     ...
     pch=spots+1,			# change the symbol (by number)
     cex.axis=1.3,			# increase the font size on the axis 
     xlab="Frog Density",		# label the x axis
     ylab="Tadpole Density",		# label the y axis
     cex.lab=1.3,			# increase the axis label font size
...
note: used github copilot during the assignment
